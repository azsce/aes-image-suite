...
if [ -f "$HOME/.zshrc" ]; then
  ...
  # Add completions to .zshrc if not already present
  if ! grep -q "bun completions" "$HOME/.zshrc"; then
    echo "" >> "$HOME/.zshrc"
    echo "# bun completions" >> "$HOME/.zshrc"
    echo "[ -s \"$HOME/.bun/_bun\" ] && source \"$HOME/.bun/_bun\"" >> "$HOME/.zshrc"
    echo "  ✅ Added Zsh completions to .zshrc"
  else
    echo "  ✔️  Zsh completions already configured"
  fi
  
  # Generate completions file
  if [ -x "$BIN_DIR/bun" ]; then
    echo "  Generating Zsh completions..."
    "$BIN_DIR/bun" completions > "$BUN_INSTALL/_bun" 2>/dev/null || echo "  ⚠️  Could not generate completions (will be created on first run)"
  fi
fi
...
if [ -f "$HOME/.bashrc" ]; then
  ...
  # Add completions to .bashrc if not already present
  if ! grep -q "bun completions" "$HOME/.bashrc"; then
    echo "" >> "$HOME/.bashrc"
    echo "# bun completions" >> "$HOME/.bashrc"
    echo "[ -s \"$HOME/.bun/_bun\" ] && source \"$HOME/.bun/_bun\"" >> "$HOME/.bashrc"
    echo "  ✅ Added Bash completions to .bashrc"
  else
    echo "  ✔️  Bash completions already configured"
  fi
fi
