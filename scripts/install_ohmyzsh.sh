...
if [ -f "$P10K_CONFIG" ]; then
    ...
    if command -v dos2unix &> /dev/null; then
        dos2unix ~/.p10k.zsh 2>/dev/null
        echo "  ✅ Line endings converted using dos2unix"
    elif command -v sed &> /dev/null; then
        sed -i 's/\r$//' ~/.p10k.zsh
        echo "  ✅ Line endings converted using sed"
    else
        echo "  ⚠️  Could not find dos2unix or sed, attempting tr method..."
        tr -d '\r' < ~/.p10k.zsh > ~/.p10k.zsh.tmp && mv ~/.p10k.zsh.tmp ~/.p10k.zsh
        echo "  ✅ Line endings converted using tr"
    fi
    
    # Set proper permissions for the p10k config file
    echo "  🔒 Setting proper permissions for .p10k.zsh..."
    chmod 644 ~/.p10k.zsh
    if [ $? -eq 0 ]; then
        echo "  ✅ Permissions set successfully (644)"
    else
        echo "  ⚠️  Failed to set permissions, but file may still work"
    fi
